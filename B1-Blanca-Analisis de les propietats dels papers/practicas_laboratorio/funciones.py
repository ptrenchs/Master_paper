...
def ejercicio_blanca(ruta, cifras_sig = 3, separador_decimales = '.'):
    ...
    for pos_tab,tabla in enumerate(tablas):
        ...
        tabla_latex = pd.DataFrame(dict(zip([i for i in tabla.columns], trans(acondicionar_tabla(tabla.values, separador_decimales = separador_decimales, cifras_sig = cifras_sig[pos_tab])))))
