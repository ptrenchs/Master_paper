...
def ejercicio_blanca(ruta, confianza = 0.95, cifras_sig = 3, separador_decimales = '.', left = '', center = '', right = '\\today'):
    ...
    for pos_tab,tabla in enumerate(tablas):
        ...
        tabla_in = schopper_corr(tabla=tabla,nombre=nombres[pos_tab],ruta = '')
        tabla_t =trans(acondicionar_tabla(tabla_in.values, separador_decimales = separador_decimales, cifras_sig = cifras_sig[pos_tab]))
        tabla_latex = pd.DataFrame(dict(zip(['muestras'] + [i for i in tabla_in.columns],[['muestra '+str(i+1) for i in range(len(trans(tabla_t)))]] + tabla_t)))
        ...
        while True:
            ...
            for lista in new_valores:
                ...
                if lista_def == []:
                    ...
                    intervalo_confianza_mes_menys.append(str(val_significativa(val = media, cifras_sig = cifras_sig[pos_tab],separador_decimales = separador_decimales)) + ' +- ' + str(val_significativa(val = mes_menys, cifras_sig = cifras_sig[pos_tab],separador_decimales = separador_decimales)))
